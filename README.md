...
- **Messages (macOS)** — the agent can send an iMessage/SMS through the Mac's Messages app, gated behind the same **Approve / Decline** card. Just ask ("text Alex that I'm running late"); nothing sends until you approve. On first use, macOS asks to let Odysseus control Messages — allow it under System Settings → Privacy & Security → Automation.
