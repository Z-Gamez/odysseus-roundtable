...
def _confine_research_path(session_id: str) -> Path:
    """Resolved path for session_id's JSON inside DEEP_RESEARCH_DIR.

    Ported from upstream odysseus-dev #4986. Validates the ID format AND
    asserts containment after symlink expansion, so traversal, absolute-path
    injection, and symlink escape all raise 400. Previously only one of the
    seven call sites validated at all, and none checked containment.
    """
